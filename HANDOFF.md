...
- **Holy Week Daily Vespers is reference content, not the actual parish service.** Holy Mon/Tue/Wed sing Presanctified Liturgy (own pre-reading prokeimena); Holy Thu sings Vesperal Liturgy of St Basil. The `/api/service?date=...` Daily Vespers form rendered for those four days is a calendar-rules artifact, not what parishes use. Calendar-rules now uses the civil-eve weekday prokeimenon there (aligned with all other generators); confirm against a parish-bulletin source at Holy Week 2027.
...
- **The orthocal-api dependency.** Calendar resolution + lectionary readings lean on an external service. Track B (2026-06-19) vendored 2025–2029 responses under `data/orthocal/`, so every in-window date hits a committed file with no external HTTP. Out-of-window dates fall through to live API with an in-process Map cache (no DB persistence). The DB-backed `orthocal_cache` table was retired in Track E (2026-06-19).
